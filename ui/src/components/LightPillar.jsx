...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      // tanh is not a built-in in GLSL ES 1.00 (Three's default ShaderMaterial),
      // so provide our own.
      vec3 tnh(vec3 x) {
        vec3 ex = exp(x);
        vec3 emx = exp(-x);
      ...
      void main() {
        ...
        col = tnh(col * uGlowAmount / widthNorm);
    ...
    const animate = (currentTime) => {
      ...
      if (deltaTime >= frameTime) {
        ...
        materialRef.current.uniforms.uRotCos.value = Math.cos(tt * 0.3)
        materialRef.current.uniforms.uRotSin.value = Math.sin(tt * 0.3)

...
// Map a tool id to a friendly icon + label for the rich card.
const TOOL_META = {
      ...
      <div className="tool-card-body">
        ...
        {result && (
          <div className="tool-field">
            ...
            <span className="f-value">{result.length > 220 ? result.slice(0, 220) + '…' : result}</span>
          </div>
        )}
      </div>
      <div className="tool-card-actions">
        <button className="btn-ghost" onClick={() => {}}>View</button>
        {result && <button className="btn-ghost" onClick={() => navigator.clipboard?.writeText(result)}>Copy Result</button>}
      </div>
    </div>
  )
}
        ...
        return (
          <div key={i} className={`msg ${isUser ? 'msg-user' : 'msg-assistant'}`}>
            <div className="msg-head">
              {!isUser && <span className="ai-badge">◆</span>}

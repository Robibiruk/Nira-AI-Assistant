...
// Adapted from react-bits "ShapeGrid" (canvas grid that lights up on hover).
// Themed to Nira: faint cyan hairline grid, glowing cyan hover fill, slow diagonal
// drift. Pointer-events stay on the canvas so it's easy to scroll the chat *over*
// it (wheel bubbles up; only mousemove is captured for the hover effect).

const BG = '#050b14' // matches --bg

export default function ShapeGrid({
  direction = 'diagonal',
  speed = 0.5,
  borderColor = 'rgba(73, 230, 255, 0.10)', // faint --glow hairline
  squareSize = 40,
  hoverFillColor = 'rgba(73, 230, 255, 0.22)', // glowing --glow hover
  shape = 'square',
  hoverTrailAmount = 0,
}) {
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // radial mask: fade grid edges into the app background (instead of #120F17)
    ...
    const handleMouseMove = (event) => {
      ...
      // Clear hover when the cursor is outside the chat area (canvas is pointer-events:none).
    ...
    // Canvas is pointer-events:none (so chat stays scrollable); track hover via a
    // window-level listener mapped to the canvas rect instead of canvas events.
    const handleWindowMove = (e) => handleMouseMove(e)
